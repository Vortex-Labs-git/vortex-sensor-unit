...
// vortex_device/sensor_unit/<DEVICE_ID>
...
static void mqtt_publish_message(const char *sub_topic, cJSON *message)
{
    ...
    // vortex_device/sensor_unit/<DEVICE_ID>/<sub_topic>
    char full_topic[128];
...
/*===============================================================
 *                PUBLISH SENSOR UNIT DATA (MAIN DATA SET)
 *==============================================================*/

/**
 * @brief Publish all sensorunit-related data:
 *        - state_data
 *        - status
 *        - error
 */
void mqtt_publish_sensorunit_data( void) 
{
    ...
    if (sensorunit_status == NULL) {
        ESP_LOGE(TAG, "Failed to create Sensor unit status");
    } else {
        mqtt_publish_message("status", sensorunit_status);
    ...
    cJSON_Delete(sensorunit_status);
    cJSON_Delete(sensorunit_error);
}
...
/**
 * @brief FreeRTOS task that publishes sensorunit data
 *        every 5 seconds.
 */
void mqtt_publish_sensorunit_data_task(void *pvParameters) {
    while (1) {
        // Publish sensorunit data
        mqtt_publish_sensorunit_data();
...
void start_mqtt_client(void)
{
    ...
    snprintf(base_topic, sizeof(base_topic), "vortex_device/sensor_unit/%s", deviceIdentity.device_id);
    ...
    // Create the sensorunit data publishing task
    BaseType_t xReturned = xTaskCreate(
        mqtt_publish_sensorunit_data_task,
        "mqtt_publish_sensorunit_data",
        4096,
        NULL,
        tskIDLE_PRIORITY + 1,
    ...
    if (xReturned != pdPASS) {
        ESP_LOGE(TAG, "Failed to create sensorunit data publishing task");
    }
